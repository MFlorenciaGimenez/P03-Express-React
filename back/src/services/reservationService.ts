...
import { reservationRepository, userRepository } from "../repositories/indexRepository";
...
export const getReservationByIdService = async (id : number) : Promise<Reservation> => {
...
export const createReservationService = async({date, time, userId}: IreservationDto): Promise<Reservation|undefined> => {
...
export const cancelReservationService = async (id:number) : Promise<Reservation> => {
    const reservation : Reservation | null = await reservationRepository.findOneBy({
        id
    })
    if(!reservation){
